...
class FetchedQuiz extends Component {
  ...
  const prevPage = () => this.setState({ currentPage: currentPage - 1 });
    ...
    return(
 <div>
   <MetaDecorator title={`flipkart quiz answers today, guess what ${this.props.curDate}`} description="webserv4u.com provide daily trivia quiz 
   answers in first instance by playing these quiz answers correctly you must win awesome prize." />
   <MetaDecorator title={`flipkart Quiz time Guess what answers today ${this.props.curDate}`} description=" Here all right
    Flipkart Daily Trivia Answers Today 1 
    are given. Start Playing Quiz by clicking on Start Quiz Button." />
  <span style={{color:"red"}}> Flipkart Daily Trivia Quiz Answers updated at
